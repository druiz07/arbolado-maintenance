...
| **TD-12** — `bump-transitive-via-overrides` **cableado en `loop.yml`**: branching `Detect playbook kind` + `Apply override` (wrapper `apply-override.mjs` IO real) + `report-bridge.js` (report unificado, +10 tests → suite 422) + issue `parent_strict_range` + PR override. Smoke E2E real verde (`25928776319`) + regresión Aider OK (`25928876870`). Adelanto proactivo | ✅ 2026-05-15 |
...
- **TD-12 (2026-05-15):** `bump-transitive-via-overrides` **cableado en `loop.yml`** + `report-bridge.js` (+10 tests → suite **422 pass / 1 skip / 0 fail**). **Smoke E2E real** (`gh run 25928776319`): seed transitive sintético → routing override → `apply-override` invoca npm real → `npm install` falla → **rollback** (repo target limpio, sin rama colgada) → issue `parent_strict_range` (#1 en este repo, cerrado tras verificación) → report `failure_stage=ci` → sin PR (correcto en fallo). **Regresión flujo Aider** (`gh run 25928876870`): la cadena Aider entra (`resolve-model` success), PR Aider dispara, steps override skipped — sin romper el flujo existente. El path `applied` lo cubren los 49 tests offline + smoke histórico (42 applied); el smoke CI ejercitó el path rollback/issue con npm real (más valioso para el edge_case).
